...
def test_openai_llm_provider_returns_none_without_api_key(
...
def test_openai_prompt_cache_retention_normalizes_old_spelling(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("OPENAI_PROMPT_CACHE_RETENTION", "in_memory")
    settings = Settings.from_env()
    assert settings.openai_prompt_cache_retention == "in-memory"
