...
type CreditLedgerEntry = {
  ...
  reservedDelta?: number; // mirrors the current reservation ledger pattern
...
Implementation note:
- this spec should reuse the existing top-level `credit_ledger` collection instead of introducing a nested `users/{userId}/credits/ledger` collection
...
Implementation note:
- because the current system already tracks `reserved` credits separately, the safest reset rule is:
  - `newBalance = monthlyAllowance`
  - `reserved` remains unchanged
  - available credits remain `balance - reserved`
...
- reset `users/{userId}.credits.balance` to the correct allowance
...
For v1, you may let the scheduler handle **all** recurring refreshes, as long as active entitlement is checked against billing state.  
That keeps billing grants separate from the existing reservation/settlement flow and reduces branching.
