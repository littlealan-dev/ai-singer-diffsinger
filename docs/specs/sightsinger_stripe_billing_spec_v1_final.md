# SightSinger Stripe Billing Integration Spec (v1, Final Revised)
...
- The refresh anchor is the date the user first enters the permanent free plan.
- For legacy non-paying users, use the original account creation timestamp as the free-plan refresh anchor.
...
- monthly refresh is granted by the daily scheduler using the user’s refresh anchor date
...
For existing legacy non-paying users:

- set `activePlanKey = free`
- use the user’s original account creation timestamp as the free-plan `creditRefreshAnchor`
- preserve the user’s current remaining credit balance until the next scheduled free-plan refresh
...
### Legacy migration
- existing non-paying users are normalized to free plan
- account creation timestamp is used as refresh anchor
- existing balance is preserved until next refresh
- next refresh resets balance to 8
...
7. implement credit ledger and credits state extensions
8. implement refresh scheduler
9. implement legacy user migration logic
10. implement pricing page integration
11. implement account/billing page
12. run end-to-end tests in Stripe test mode
13. launch soft rollout
