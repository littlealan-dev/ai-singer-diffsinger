...
: "${LLM_PROVIDER:=static}"
: "${LLM_STATIC_LOOP:=true}"
DEFAULT_STATIC_RESPONSE=$(
  cat <<'JSON'
[{"tool_calls":[{"name":"estimate_credits","arguments":{}}],"final_message":"Estimating credits...","include_score":false},{"tool_calls":[],"final_message":"Estimated credits ready. Proceed?","include_score":false},{"tool_calls":[{"name":"synthesize","arguments":{}}],"final_message":"Starting synthesis.","include_score":false}]
JSON
)
...
export LLM_PROVIDER
export LLM_STATIC_RESPONSE
export LLM_STATIC_LOOP
