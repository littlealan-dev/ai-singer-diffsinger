...
LINT_RULE_SPECS: Dict[str, LintRuleSpec] = {
    ...
    "complete_split_source_underclaimed": LintRuleSpec(
        code="complete_split_source_underclaimed",
        name="Complete Split Source Underclaimed",
        severity="P0",
        domain="STRUCTURAL",
        definition="A complete split must export enough distinct derived lanes to carry every simultaneous note in each source voice.",
        fail_condition="The plan marks a source as complete but fewer output lanes claim a chordal source measure than its simultaneous-note count.",
        suggestion="Return a complete full-song plan with additional distinct output lanes for the reported source and ranges.",
        message_template="Complete split does not provide enough distinct derived lanes for this chordal source range.",
    ),
    "complete_split_duplicate_rank": LintRuleSpec(
        code="complete_split_duplicate_rank",
        name="Complete Split Duplicate Rank",
        severity="P0",
        domain="STRUCTURAL",
        definition="Derived lanes in a complete chord split must select distinct chord ranks at the same source range.",
        fail_condition="Two or more output lanes select the same ranked chord note while a different required rank remains unclaimed.",
        suggestion="Keep the full-song plan and change only the reported source sections so each output lane selects a distinct rank.",
        message_template="Complete split assigns the same chord rank to multiple derived lanes.",
    ),
    "complete_split_scope_lane_count_mismatch": LintRuleSpec(
        code="complete_split_scope_lane_count_mismatch",
        name="Complete Split Scope Lane Count Mismatch",
        severity="P0",
        domain="STRUCTURAL",
        definition="A complete split must create exactly one visible derived lane for each simultaneous note in its staff scope at the scope's maximum density.",
        fail_condition="A complete-split staff scope has a visible derived-lane count different from the scope's maximum simultaneous source-note count.",
        suggestion="Return a complete full-song plan with exactly the required number of visible derived lanes for the reported staff scope, while keeping explicit selective requests separate.",
        message_template="Complete split derived-lane count does not match the maximum simultaneous note count in this staff scope.",
    ),
    ...
    "derived_part_source_for_original_target": LintRuleSpec(
        code="derived_part_source_for_original_target",
        name="Derived Part Source For Original Target",
        severity="P0",
        domain="STRUCTURAL",
        definition="When preparing an original score part, melody and lyric sources must come from the original part set, not from previously appended derived outputs.",
        fail_condition="A derive section targets an original part but uses an already-derived part as its melody_source or lyric_source.",
        suggestion="Use the target part's own voice parts as melody/lyric sources, then add solfege to the newly derived output after preprocessing succeeds.",
        message_template="Plan uses a previously derived part as the source for an original target part.",
    ),
    "generated_solfege_lyric_source": LintRuleSpec(
        code="generated_solfege_lyric_source",
        name="Generated Solfege Lyric Source",
        severity="P0",
        domain="LYRIC",
        definition="SightSinger-generated solfege lyrics are post-preprocess output and must not be used as lyric_source input for another preprocess plan.",
        fail_condition="A derive section selects a lyric_source whose source notes include lyrics named `SightSinger Solfege`.",
        suggestion="Choose a non-generated lyric_source from the original score material, or split the part first and run add_solfege_lyric_verse on the derived output afterward.",
        message_template="Plan uses SightSinger-generated solfege lyrics as a lyric source.",
    ),
