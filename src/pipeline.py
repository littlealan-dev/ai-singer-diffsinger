...
class PhraseData:
    ...
    input_word_div: List[int]
    input_word_dur: List[int]
...
class PitchContext:
    ...
    ph_durations: np.ndarray
    note_dur: List[int]
    note_midi: np.ndarray
    note_rest: np.ndarray
    base_midi: np.ndarray
    n_frames: int
...
class TimeAxis:
    ...
    def __init__(self, tempos: List[TempoEvent]):
        ...
        Outputs: instance with cached ms offsets per tempo change.
        """
    ...
    def get_ms_at_beat(self, beat: float) -> float:
        """Convert a beat position to elapsed milliseconds.
        Inputs: beat (float).
        Outputs: elapsed time in ms (float).
        """
        # Find active tempo (linear scan is fine for typical scores).
        idx = 0
...
class Pipeline:
    ...
    def _load_pipeline_config(self) -> PipelineConfig:
        """Load dsconfig.yaml and map it into PipelineConfig.
        Inputs: none (uses self.root).
        Outputs: PipelineConfig instance.
        """
    ...
    def _init_phonemizer(self) -> Phonemizer:
        """Initialize the phonemizer from voicebank metadata.
        Inputs: none (uses self.root).
        Outputs: Phonemizer instance.
        """
    ...
    def _init_linguistic(self) -> LinguisticModel:
        """Load the linguistic encoder model.
        Inputs: none (uses self.root).
        Outputs: LinguisticModel instance.
        """
        # Try dsdur first.
    ...
    def _init_duration(self) -> DurationModel:
        ...
        Inputs: none (uses self.root).
        Outputs: DurationModel instance.
        """
        dsdur_path = self.root / "dsdur"
        if dsdur_path.exists():
            conf = yaml.safe_load((dsdur_path / "dsconfig.yaml").read_text())
    ...
    def _init_pitch(self) -> Optional[PitchModel]:
        """Load the pitch model if available.
        Inputs: none (uses self.root).
        Outputs: PitchModel instance or None.
        """
    ...
    def _init_pitch_linguistic(self) -> Optional[LinguisticModel]:
        """Load the pitch-side linguistic encoder if available.
        Inputs: none (uses self.root).
        Outputs: LinguisticModel instance or None.
        """
        dspitch_path = self.root / "dspitch"
        if dspitch_path.exists():
            conf = yaml.safe_load((dspitch_path / "dsconfig.yaml").read_text())
    ...
    def _init_variance(self) -> Optional[VarianceModel]:
        """Load the variance model if available.
        Inputs: none (uses self.root).
        Outputs: VarianceModel instance or None.
        """
    ...
    def _init_variance_linguistic(self) -> Optional[LinguisticModel]:
        """Load the variance-side linguistic encoder if available.
        Inputs: none (uses self.root).
        Outputs: LinguisticModel instance or None.
        """
        dsvariance_path = self.root / "dsvariance"
    ...
    def _init_acoustic(self) -> AcousticModel:
        ...
        Inputs: none (uses self.root).
        Outputs: AcousticModel instance.
        """
        conf = yaml.safe_load((self.root / "dsconfig.yaml").read_text())
        if "acoustic" in conf:
    ...
    def _resolve_dictionary_path(self) -> Path:
        ...
        Inputs: none (uses self.root).
        Outputs: Path to an existing dictionary.
        """
        candidates = [
    ...
    def _load_speaker_embed(self, name: Optional[str] = None) -> np.ndarray:
        """Load a speaker embedding vector.
        Inputs: optional name filter (str).
        Outputs: 1D numpy array of float32.
        """
        conf = yaml.safe_load((self.root / "dsconfig.yaml").read_text())
    ...
    def _repeat_embed(embed: np.ndarray, length: int) -> np.ndarray:
        """Repeat a speaker embedding across time frames.
        Inputs: embed (1D np.ndarray), length (int).
        Outputs: (1, length, dim) array.
        """
        if embed.ndim != 1:
    ...
    def _build_ph2word(word_div: List[int]) -> np.ndarray:
        """Build a phoneme-to-word index mapping.
        Inputs: word_div list of phoneme counts per word.
        Outputs: 1D int64 array of word indices (1-based).
        """
        ph2word: List[int] = []
    ...
    def _align_durations(
        ...
        """Scale phoneme durations to match word durations.
        Inputs: predicted phoneme durations, word_div, word_dur.
        Outputs: aligned phoneme durations (int64).
        """
    ...
    def _expand_by_duration(values: List[float], durations: np.ndarray) -> np.ndarray:
        """Repeat each value by its duration count.
        Inputs: values list, durations array.
        Outputs: expanded float32 array.
        """
        expanded: List[float] = []
    ...
    def _fill_rest_midi(note_midi: np.ndarray, note_rest: np.ndarray) -> np.ndarray:
        """Interpolate MIDI across rests for pitch continuity.
        Inputs: note_midi array, note_rest bool array.
        Outputs: filled MIDI float32 array.
        """
        if note_midi.size == 0:
            return note_midi
        if note_rest.all():
    ...
    def _dump_debug(debug_dir: Optional[Path], name: str, array: np.ndarray) -> None:
        """Persist a debug numpy array if debugging is enabled.
        Inputs: debug_dir (Path or None), name (str), array (np.ndarray).
        Outputs: none (side effect: writes .npy file).
        """
    ...
    def _resolve_debug_dir(
        ...
        """Resolve where to write debug outputs.
        Inputs: debug_dir, output_path.
    ...
    def _select_voice_notes(
        ...
        """Filter notes to a single voice within a part.
        Inputs: notes list, voice_id (optional).
        Outputs: filtered list of NoteEvent.
        """
        ...
        if selected_voice is not None:
            return [note for note in notes if note.voice == selected_voice]
        if voice_pitches:
            return [note for note in notes if note.voice is not None]
        return notes

    def _compute_note_timing(self, notes: List[NoteEvent], time_axis: TimeAxis) -> NoteTiming:
        """Convert note beat positions into frame timings.
        Inputs: notes list, time_axis.
        Outputs: NoteTiming with start/end frames, MIDI, rest flags.
        """
        start_frames: List[int] = []
        end_frames: List[int] = []
        midi_raw: List[float] = []
        ...
        for note in notes:
            ...
            if note.is_rest:
                ...
                midi_raw.append(float(note.pitch_midi or 0.0))

        midi = self._fill_rest_midi(
            np.array(midi_raw, dtype=np.float32),
            np.array(rest_flags, dtype=bool),
        )
        return NoteTiming(
    ...
    def _build_phrase_groups(
        ...
        """Build phoneme groups and word durations from notes.
        Inputs: notes list, timing.
        Outputs: PhraseData with grouped phonemes and timings.
        """
        # OpenUtau-style phoneme distribution: consonants attach to the
        # previous note group, vowels open a new note group.
        ...
        phrase_groups: List[Dict[str, Any]] = [
            {
                "position": timing.start_frames[0] - padding_frames,
                "phonemes": ["SP"],
                "ids": [sp_id],
                "lang_ids": [0],
                "tone": float(timing.midi[0]),
                "note_idx": None,
            }
        ]
        note_phonemes: Dict[int, List[str]] = {}
        ...
        for group in word_groups:
            ...
            if group["is_rest"]:
                ...
                phrase_groups.append(
                    {
                        "position": timing.start_frames[note_idx],
                        "phonemes": ["SP"],
                        "ids": [sp_id],
                        "lang_ids": [0],
                        "tone": float(timing.midi[note_idx]),
                        "note_idx": note_idx,
                    }
                )
        ...
        phrase_groups.append(
            {
                "position": timing.end_frames[-1],
                "phonemes": [],
                "ids": [],
                "lang_ids": [],
                "tone": float(timing.midi[-1]),
                "note_idx": None,
            }
        )
        ...
        return PhraseData(
            ...
            input_word_div=input_word_div,
            input_word_dur=input_word_dur,
            note_phonemes=note_phonemes,
            sp_id=sp_id,
        )

    def _run_linguistic(self, phrase: PhraseData) -> LinguisticResult:
        """Run the linguistic encoder model.
        Inputs: PhraseData.
        Outputs: LinguisticResult with encoder outputs and masks.
        """
        tokens_tensor = np.array(phrase.input_tokens, dtype=np.int64)[None, :]
        languages_tensor = np.array(phrase.input_languages, dtype=np.int64)[None, :]
        word_div_tensor = np.array(phrase.input_word_div, dtype=np.int64)[None, :]
        word_dur_tensor = np.array(phrase.input_word_dur, dtype=np.int64)[None, :]
    ...
    def _predict_durations(
        ...
        """Predict phoneme durations from linguistic features.
        Inputs: LinguisticResult, PhraseData.
        Outputs: DurationResult with aligned durations.
        """
        ph_midi: List[int] = []
        for group in phrase.phrase_groups[:-1]:
            tone = int(round(group["tone"]))
            ph_midi.extend([tone] * len(group["ids"]))
        ph_midi_tensor = np.array(ph_midi, dtype=np.int64)[None, :]

        spk_embed_tokens = self._repeat_embed(self.spk_embed, linguistic.tokens.shape[1]).astype(np.float32)
        duration_out = self.duration.forward(
            linguistic.encoder_out,
            linguistic.x_masks,
            ph_midi_tensor,
            spk_embed_tokens,
        )
        ph_dur_pred = duration_out[0]
        ph_durations = self._align_durations(
            ph_dur_pred,
            phrase.input_word_div,
            phrase.input_word_dur,
        )
        return DurationResult(
            ph_midi=ph_midi_tensor,
            ph_midi_list=ph_midi,
            ph_dur_pred=ph_dur_pred,
            ph_durations=ph_durations,
        )

    def _prepare_pitch_context(
        self,
        notes: List[NoteEvent],
        timing: NoteTiming,
        phrase: PhraseData,
        ...
        """Assemble pitch inputs for the pitch/variance models.
        Inputs: notes, timing, phrase, ph_durations.
        Outputs: PitchContext with token/phoneme and note timing arrays.
        """
        ...
        pitch_tokens_tensor = np.array(phrase.input_tokens + [phrase.sp_id], dtype=np.int64)[None, :]
        pitch_languages_tensor = np.array(phrase.input_languages + [0], dtype=np.int64)[None, :]

        start_frame = timing.start_frames[0] - self.HEAD_FRAMES
        note_dur = [max(1, timing.start_frames[0] - start_frame)]
        for idx in range(len(timing.start_frames) - 1):
            note_dur.append(max(1, timing.start_frames[idx + 1] - timing.start_frames[idx]))
        note_dur.append(0)
        note_dur[-1] = max(1, n_frames - sum(note_dur[:-1]))
        note_midi_pitch = np.concatenate(([timing.midi[0]], timing.midi)).astype(np.float32)
        ...
        for idx, note in enumerate(notes):
            ...
            if is_extension and idx > 0:
                ...
                continue
            phs = phrase.note_phonemes.get(idx, [])
        ...
        return PitchContext(
            pitch_tokens=pitch_tokens_tensor,
            pitch_languages=pitch_languages_tensor,
            ph_durations=ph_durations_pitch,
            note_dur=note_dur,
            note_midi=note_midi_pitch,
            note_rest=note_rest_pitch,
            base_midi=base_midi,
            n_frames=n_frames,
        )
    ...
    def _predict_pitch(
        ...
        """Predict pitch curve and F0 from pitch context.
        Inputs: PitchContext, encoder_out, ph_midi_list.
        ...
        if self.pitch:
            ...
            if self.pitch_linguistic is not None:
                pitch_ling_inputs = {
                    "tokens": pitch_ctx.pitch_tokens,
                    "languages": pitch_ctx.pitch_languages
                    if self.config.use_lang_id
                    else np.zeros_like(pitch_ctx.pitch_tokens),
                    "ph_dur": pitch_ctx.ph_durations[None, :].astype(np.int64),
                }
                pitch_encoder_out = self.pitch_linguistic.run(pitch_ling_inputs)[0]
            pitch_inputs = {
                "encoder_out": pitch_encoder_out,
                "ph_dur": pitch_ctx.ph_durations[None, :].astype(np.int64),
                "note_midi": pitch_ctx.note_midi[None, :].astype(np.float32),
                "note_rest": pitch_ctx.note_rest[None, :],
                "note_dur": np.array(pitch_ctx.note_dur, dtype=np.int64)[None, :],
                "pitch": np.full((1, pitch_ctx.n_frames), 60.0, dtype=np.float32),
                "expr": expr,
                "retake": retake,
            ...
            return pitch_pred, pitch_midi, f0

        ph_midi_pitch = [float(ph_midi_list[0])] + [float(m) for m in ph_midi_list[1:]] + [float(ph_midi_list[-1])]
        pitch_midi = self._expand_by_duration(ph_midi_pitch, pitch_ctx.ph_durations)[None, :]
        f0 = 440.0 * (2.0 ** ((pitch_midi - 69.0) / 12.0))
        return None, pitch_midi, f0

    def _predict_variance(
        self,
        pitch_ctx: PitchContext,
        encoder_out: np.ndarray,
        pitch_semitone: np.ndarray,
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Predict variance features from pitch context.
        Inputs: PitchContext, encoder_out, pitch_semitone.
        Outputs: breathiness, voicing, tension arrays.
        """
        if not self.variance:
            zeros = np.zeros((1, pitch_ctx.n_frames), dtype=np.float32)
        ...
        variance_inputs = {
            ...
            "breathiness": np.zeros((1, pitch_ctx.n_frames), dtype=np.float32),
            "voicing": np.zeros((1, pitch_ctx.n_frames), dtype=np.float32),
            "tension": np.zeros((1, pitch_ctx.n_frames), dtype=np.float32),
            "retake": np.ones((1, pitch_ctx.n_frames, num_variances), dtype=bool),
            "spk_embed": spk_embed_frames,
            "steps": np.array(self.config.steps, dtype=np.int64),
        }
        if self.variance_predict_energy:
            variance_inputs["energy"] = np.zeros((1, pitch_ctx.n_frames), dtype=np.float32)
    ...
    def _run_acoustic(
        ...
        """Run the acoustic model to produce mel features.
        Inputs: PitchContext, variance features, f0.
        Outputs: mel spectrogram array.
        """
        spk_embed_frames = self._repeat_embed(self.spk_embed, pitch_ctx.n_frames).astype(np.float32)
        ...
        acoustic_inputs = {
            ...
            "durations": pitch_ctx.ph_durations[None, :].astype(np.int64),
            "f0": f0.astype(np.float32),
            "breathiness": breathiness.astype(np.float32),
            "voicing": voicing.astype(np.float32),
            "tension": tension.astype(np.float32),
            "gender": np.zeros((1, pitch_ctx.n_frames), dtype=np.float32),
            "velocity": np.ones((1, pitch_ctx.n_frames), dtype=np.float32),
            "spk_embed": spk_embed_frames,
            "depth": np.array(depth, dtype=np.float32),
            "steps": np.array(self.config.steps, dtype=np.int64),
        }
        return self.acoustic.run(acoustic_inputs)[0]
    ...
    def infer(
        ...
        *,
        ...
        """Run the full pipeline from MusicXML to audio.
        Inputs: score_path, output_path (optional), debug_dir, voice_id, stop_after.
        Outputs: writes output wav when stop_after is None; returns None.
        """
        # Parse the score and build a tempo-aware time axis.
        ...
        # Select the target part/voice to synthesize.
        notes = score.parts[0].notes
        if not notes:
            raise ValueError("No notes found in the selected MusicXML part.")

        notes = self._select_voice_notes(notes, voice_id)
        if not notes:
            raise ValueError("No notes left after applying voice selection.")

        # Convert note beats into frame timing and build phrase groups.
        timing = self._compute_note_timing(notes, time_axis)
        phrase = self._build_phrase_groups(notes, timing)
        linguistic = self._run_linguistic(phrase)

        # Linguistic stage outputs.
        self._dump_debug(debug_root, "tokens", linguistic.tokens)
        self._dump_debug(debug_root, "languages", linguistic.languages)
        self._dump_debug(debug_root, "word_div", linguistic.word_div)
        self._dump_debug(debug_root, "word_dur", linguistic.word_dur)
        self._dump_debug(debug_root, "encoder_out", linguistic.encoder_out)
        self._dump_debug(debug_root, "x_masks", linguistic.x_masks.astype(np.int8))
        if stop_after == "linguistic":
            return

        # Duration prediction and alignment.
        durations = self._predict_durations(linguistic, phrase)
        self._dump_debug(debug_root, "ph_midi", durations.ph_midi)
        self._dump_debug(debug_root, "ph_dur_pred", durations.ph_dur_pred.astype(np.float32))
        self._dump_debug(debug_root, "ph_durations", durations.ph_durations.astype(np.int64))
        if stop_after == "duration":
            return

        # Pitch preparation and prediction.
        pitch_ctx = self._prepare_pitch_context(notes, timing, phrase, durations.ph_durations)
        pitch_pred, pitch_midi, f0 = self._predict_pitch(
            pitch_ctx,
            linguistic.encoder_out,
            durations.ph_midi_list,
        )
        pitch_semitone = pitch_midi.astype(np.float32)

        # Pitch stage outputs.
        self._dump_debug(debug_root, "note_midi", pitch_ctx.note_midi)
        self._dump_debug(debug_root, "note_rest", pitch_ctx.note_rest)
        self._dump_debug(debug_root, "note_dur", np.array(pitch_ctx.note_dur, dtype=np.int64))
        self._dump_debug(debug_root, "base_midi", pitch_ctx.base_midi)
        ...
        # Variance prediction (energy/breathiness/voicing/tension).
        breathiness, voicing, tension = self._predict_variance(
            pitch_ctx,
            linguistic.encoder_out,
            pitch_semitone,
        )
        ...
        mel = self._run_acoustic(pitch_ctx, breathiness, voicing, tension, f0)
        self._dump_debug(debug_root, "mel", mel.astype(np.float32))
        if stop_after == "acoustic":
            return

        # Vocoder to waveform and write output.
    ...
    def _naive_pitch(self, ph_midi: List[int], ph_durations: np.ndarray) -> np.ndarray:
        """Build a flat pitch curve without a pitch model.
        Inputs: ph_midi list, ph_durations array.
        Outputs: f0 array.
        """
